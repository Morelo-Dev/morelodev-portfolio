...
const knowledge: KnowledgeEntry[] = [
  ...
  // ── JM Fénix ───────────────────────────────────────────────────────────────
  {
    ...
    answer: {
      es: 'JM Fénix es el proyecto principal de Jorge — un SaaS tributario para contadores y firmas contables en Colombia 🚀\n\nFuncionalidades:\n• Gestión de clientes (naturales y jurídicos) con perfil 360°\n• Obligaciones tributarias y calendarios DIAN integrados\n• Pasarela de pagos Wompi con licenciamiento automático\n• Control de roles y permisos granular\n• Notificaciones automáticas por email (Resend)\n• Jobs recurrentes con Hangfire\n• Auditoría completa del sistema\n• PWA instalable\n\nFrontend: React 19 + Vite 7 + TypeScript + Tailwind + Zustand + shadcn/ui\nBackend: .NET 10 + ASP.NET Core + PostgreSQL (Supabase) + Redis (Upstash) + Firebase Auth\nEstado: En desarrollo activo — disponible en jmfenix.morelodev.com',
      en: "JM Fénix is Jorge's main project — a tax SaaS for accountants and accounting firms in Colombia 🚀\n\nFeatures:\n• Client management (individuals & legal entities) with 360° profile\n• DIAN tax obligations and fiscal calendars\n• Wompi payment gateway with automatic licensing\n• Granular role and permission control\n• Automated email notifications (Resend)\n• Recurring jobs with Hangfire\n• Complete system audit trail\n• Installable PWA\n\nFrontend: React 19 + Vite 7 + TypeScript + Tailwind + Zustand + shadcn/ui\nBackend: .NET 10 + ASP.NET Core + PostgreSQL (Supabase) + Redis (Upstash) + Firebase Auth\nStatus: Actively in development — live at jmfenix.morelodev.com",
    },
  },
